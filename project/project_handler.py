...
def projects_handler_init(bot):
    ...
    def handle_project_details(call):
        ...
        response = get_project_details(project_id)  # Выполняем запрос
    ...
    def handle_project_statistics(call):
        ...
        response = get_project_statistics(project_id)  # Выполняем запрос
    ...
    def handle_project_commits(call):
        ...
        response = get_project_commits(project_id, iso_format_time)  # Выполняем запрос
        if response.status_code == 200:
