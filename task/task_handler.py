...
def handle_project_new_task(call):
    ...
    bot.register_next_step_handler(call.message, handle_task_name, project_id)


def handle_task_name(message, project_id):
    ...
    bot.register_next_step_handler(
        message, handle_task_description, project_id, task_name
    )


def handle_task_description(message, project_id, task_name):
...
def handle_task_deadline(message, project_id, task_name, task_description):
    ...
    try:
        ...
        response = add_task_to_project(project_id, new_task_data)  # Выполняем запрос
...
def handle_project_tasks(call):
    ...
    try:
        tasks = get_project_tasks(project_id)  # Выполняем запрос
